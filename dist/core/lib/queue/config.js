...
export const DEFAULT_QUEUE_CONFIGURATION = deepFreeze(structuredClone(defaultConfig));
export function createQueueConfiguration(overrides = {}) {
    const merged = mergeDeep(DEFAULT_QUEUE_CONFIGURATION, overrides);
    const sanitized = sanitizeConfiguration(merged);
    return sanitized;
}
export function validateQueueConfiguration(config) {
    const errors = [];
    const warnings = [];
    ...
    if (!Number.isFinite(config.monitoring.metricsInterval) ||
    ...
    if (!Number.isFinite(thresholds.failureRate) ||
    ...
    if (!Number.isFinite(thresholds.diskUsage) ||
    ...
    if (!Number.isFinite(config.performance.processingInterval) ||
