...
使用mcp工具“deepwiki”查询的时候经常查询的内容太多导致超过限制，如何处理？
...
1. 使用mcp工具deepwiki 来随时查询https://github.com/modelcontextprotocol/typescript-sdk
   的内容,将当前仓库的MCP server部分进行改造,改造成使用typescript-sdk来完成MCP
   server开发,并发布npm 服务器.
...
既然涉及多个角色，浮浮酱想了解权限分配喵～

🔐 角色权限：

- 架构师：完全编辑权限？---yes
- 产品经理：可以编辑业务需求部分？还是只能查看+评论？---yes
- 开发团队/测试团队：只读+评论权限？---yes
...
- 是否允许添加新的技术架构字段？（如：性能指标、安全考虑）--暂时不考虑性能和安全
- 章节结构可以调整吗？（重排序、添加子章节） ---yes
- 决策对比表格的格式可以自定义吗？---yes
...
- 是否需要与Git仓库集成？（版本控制、分支管理） ---no
- 需要与项目管理工具集成吗？（如：JIRA、Notion） ---no
- 是否要集成图表工具？（架构图、流程图） ---yes，mermaid图表和 ASCII文本画图

📊 数据导入导出：

- 需要导出为其他格式吗？（PDF、Word、HTML） ---no
- 是否支持从现有文档导入？ ---option
