...
export class DataValidator {
  ...
  static validateSemver(value: string, field = 'version'): ValidationResult {
    ...
    const errors: ValidationError[] = valid ? [] : [{ field, message: 'invalid semver', code: 'VAL_SEMVER' }];
    return { valid, errors, warnings: [] };
  }

  static validateUniqueId(id: string, existing: Set<string>, field = 'id'): ValidationResult {
    const valid = id !== '' && !existing.has(id);
    const errors: ValidationError[] = valid ? [] : [{ field, message: 'id must be unique', code: 'VAL_UNIQUE' }];
    return { valid, errors, warnings: [] };
  }
  ...
  static detectCycles(nodes: string[], edges: Array<[string, string]>): string[][] {
    ...
    function dfs(n: string, path: string[]) {
      ...
      visited.add(n);
      stack.add(n);
      for (const m of graph.get(n) || []) dfs(m, path.concat(m));
      stack.delete(n);
    }
  ...
  static validateAgainstSchema(obj: Record<string, any>, schema: Record<string, { type: string; required?: boolean }>): ValidationResult {
    ...
    for (const [k, s] of Object.entries(schema)) {
      ...
      if (v !== undefined && v !== null) {
        ...
        if (t !== s.type) errors.push({ field: k, message: `expected ${s.type}`, code: 'VAL_TYPE' });
      }
    }
