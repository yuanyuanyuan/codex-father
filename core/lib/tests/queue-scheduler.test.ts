...
describe('TaskScheduler (T068 scheduler)', () => {
  ...
  afterEach(() => { vi.useRealTimers(); rmSync(base, { recursive: true, force: true }); });

  it('promotes scheduled tasks and executes pending tasks', async () => {
    // scheduled task due now
    const { taskId: sId } = await ops.enqueueTask({ type: 'data:processing', priority: 1, payload: { data: [1] }, scheduledAt: new Date() });
    // pending task
    const { taskId: pId } = await ops.enqueueTask({ type: 'data:processing', priority: 1, payload: { data: [2] } });
