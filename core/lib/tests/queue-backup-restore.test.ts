...
const REQUIRED_DIRS = [
  'pending/tasks','pending/metadata','running/tasks','running/metadata','retrying/tasks','retrying/metadata','completed/tasks','completed/metadata','failed/tasks','failed/metadata','timeout/tasks','timeout/metadata','cancelled/tasks','cancelled/metadata','locks','logs','tmp'
];
...
describe('QueueBackupManager (T067 backup/restore)', () => {
  ...
  beforeEach(() => {
    ...
    writeFileSync(join(base, 'pending/tasks', 'a.json'), JSON.stringify({ id: 'a', status: 'pending' }), 'utf8');
  });
