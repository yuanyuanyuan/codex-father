...
export class QueueMonitor {
  ...
  private evaluateAlerts(stats: Awaited<ReturnType<QueueStatisticsCollector['collect']>>, config?: QueueConfiguration): string[] {
    const alerts: string[] = [];
    if (!config?.monitoring?.enabled) return alerts;
    ...
    if (queueDepth > th.queueDepth) alerts.push(`queueDepth>${th.queueDepth}`);
    if (failureRate > th.failureRate) alerts.push(`failureRate>${th.failureRate}%`);
    if (avgWait > th.averageWaitTime) alerts.push(`averageWaitTime>${th.averageWaitTime}ms`);
    if (diskUsage > th.diskUsage) alerts.push(`diskUsage>${th.diskUsage}%`);
    return alerts;
  }
}
