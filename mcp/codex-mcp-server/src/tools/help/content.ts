...
export const guideContent: Record<(typeof canonicalOrder)[number], GuideMeta> = {
  ...
  'codex.exec': {
    ...
    tips: [
      ...
      'MCP tools/call 示例（规范旗标）：{"name":"codex.exec","arguments":{"args":["--preset","analysis","--task","检查覆盖率","--context-grep","(README|CHANGELOG)"]}}',
    ],
  ...
  'codex.start': {
    ...
    tips: [
      ...
      '上下文体积预检：默认 INPUT_TOKEN_LIMIT=32000；超过将立即失败 (exit 2)，classification=context_overflow。建议拆分任务或仅传摘要；必要时可临时提高上限。',
      '未知预设将直接失败：--preset 仅允许 sprint|analysis|secure|fast，传递其他值会以 input_error 结束。',
      'MCP tools/call 示例（规范旗标）：{"name":"codex.start","arguments":{"args":["--preset","sprint","--task","修复 T003：补齐迁移脚本并通过测试","--context-head","200"],"tag":"t003"}}',
    ],
    returnsJsonString: true,
  },
  'codex.resume': {
...
export const onboardingTips: string[] = [
  ...
  '预设严格校验：--preset 仅允许 sprint|analysis|secure|fast；未知值会触发 input_error。',
];
