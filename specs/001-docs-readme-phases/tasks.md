...
- **估时**: 3小时
- **依赖**: 无
- **状态**: COMPLETED - 定义接口与循环依赖检测、SemVer 校验与唯一性校验；配套单元测试覆盖
...
- **估时**: 3小时
- **依赖**: 无
- **状态**: COMPLETED - 任务状态流转与重试退避（fixed/linear/exponential）；含单元测试
...
- **估时**: 3小时
- **依赖**: T031-T037
- **状态**: COMPLETED - 提供 SemVer、唯一性、循环检测与极简 Schema 校验；含单元测试
