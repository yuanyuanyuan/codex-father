...
describe('PRD完整工作流程集成测试', () => {
  ...
  test('完整工作流程：配置→创建→编辑→审查', async () => {
    ...
    execSync('prd config set user.email "zhangsan@example.com"', {
      cwd: testDir,
      env: process.env,
    });
    ...
    // 4. 添加技术决策和图表 (3分钟场景)
    // 添加架构图
    const architectureEditOutput = execSync(`prd edit ${prdId} --section architecture`, {
      cwd: testDir,
      encoding: 'utf8',
      env: process.env,
    });
  ...
  test('性能基准：操作响应时间验证', async () => {
    ...
    const createOutput = execSync('prd create --title "性能测试PRD" --template technical', {
      cwd: testDir,
      encoding: 'utf8',
      env: process.env,
    });
