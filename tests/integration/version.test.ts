...
describe('PRD版本管理集成测试', () => {
  ...
  test('版本对比功能', async () => {
    // 创建PRD并进行多次编辑
    const createOutput = execSync('prd create --title "版本对比测试" --template business', {
      cwd: testDir,
      encoding: 'utf8',
      env: process.env,
    });
    ...
    // 编辑生成多个版本
    execSync(`prd edit ${prdId} --section requirements --message "添加核心业务需求"`, {
      cwd: testDir,
      env: process.env,
    });

    execSync(`prd edit ${prdId} --section market --message "添加市场分析和竞品对比"`, {
      cwd: testDir,
      env: process.env,
    });

    execSync(`prd edit ${prdId} --section timeline --message "制定详细开发时间线"`, {
      cwd: testDir,
      env: process.env,
    });
  ...
  test('版本回滚功能', async () => {
    ...
    // 创建多个版本
    execSync(`prd edit ${prdId} --section feature --message "添加功能详细描述"`, {
      cwd: testDir,
      env: process.env,
    });

    execSync(`prd edit ${prdId} --section acceptance --message "添加验收标准"`, {
      cwd: testDir,
      env: process.env,
    });

    // 错误的编辑 - 需要回滚
    execSync(`prd edit ${prdId} --section implementation --message "错误的实施方案，需要回滚"`, {
      cwd: testDir,
      env: process.env,
    });
  ...
  test('版本查看和内容访问', async () => {
    // 创建PRD
    const createOutput = execSync('prd create --title "历史版本查看测试" --template technical', {
      cwd: testDir,
      encoding: 'utf8',
      env: process.env,
    });
    ...
    execSync(`prd edit ${prdId} --section architecture --message "${version2Content}"`, {
      cwd: testDir,
      env: process.env,
    });

    const version3Content = '版本3的安全考虑内容';
    execSync(`prd edit ${prdId} --section security --message "${version3Content}"`, {
      cwd: testDir,
      env: process.env,
    });
  ...
  test('版本元数据和统计信息', async () => {
    // 创建PRD
    const createOutput = execSync('prd create --title "版本统计测试" --template business', {
      cwd: testDir,
      encoding: 'utf8',
      env: process.env,
    });
    ...
    execSync(`prd edit ${prdId} --section architecture --message "架构师的技术设计"`, {
      cwd: testDir,
      env: process.env,
    });
    ...
    execSync(`prd edit ${prdId} --section requirements --message "产品经理的需求细化"`, {
      cwd: testDir,
      env: process.env,
    });
    ...
    execSync(`prd edit ${prdId} --section testing --message "测试工程师的测试计划"`, {
      cwd: testDir,
      env: process.env,
    });
  ...
  test('版本错误处理和边界情况', async () => {
    const createOutput = execSync('prd create --title "版本错误测试" --template feature', {
      cwd: testDir,
      encoding: 'utf8',
      env: process.env,
    });
  ...
  test('版本性能要求验证', async () => {
    // 创建PRD
    const createOutput = execSync('prd create --title "版本性能测试" --template technical', {
      cwd: testDir,
      encoding: 'utf8',
      env: process.env,
    });
    ...
    // 创建多个版本
    for (let i = 2; i <= 10; i++) {
      execSync(`prd edit ${prdId} --section test-section-${i} --message "版本${i}的内容"`, {
        cwd: testDir,
        env: process.env,
      });
    }
