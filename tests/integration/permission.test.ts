...
describe('PRD权限管理集成测试', () => {
  ...
  test('architect角色：完整权限验证', async () => {
    ...
    // architect应该能够：
    // 1. 创建所有类型的PRD
    const technicalPrd = execSync('prd create --title "技术架构PRD" --template technical', {
      cwd: testDir,
      encoding: 'utf8',
      env: process.env,
    });
    ...
    const businessPrd = execSync('prd create --title "业务需求PRD" --template business', {
      cwd: testDir,
      encoding: 'utf8',
      env: process.env,
    });
  ...
  test('product_manager角色：业务权限验证', async () => {
    ...
    const businessPrd = execSync('prd create --title "产品需求PRD" --template business', {
      cwd: testDir,
      encoding: 'utf8',
      env: process.env,
    });
    ...
    const featurePrd = execSync('prd create --title "功能规格PRD" --template feature', {
      cwd: testDir,
      encoding: 'utf8',
      env: process.env,
    });
  ...
  test('developer角色：开发权限验证', async () => {
    ...
    execSync('prd config set user.role "architect"', { cwd: testDir, env: process.env });
    const createOutput = execSync('prd create --title "开发测试PRD" --template technical', {
      cwd: testDir,
      encoding: 'utf8',
      env: process.env,
    });
  ...
  test('tester角色：测试权限验证', async () => {
    ...
    // 先创建PRD供测试
    execSync('prd config set user.role "architect"', { cwd: testDir, env: process.env });
    const createOutput = execSync('prd create --title "测试PRD" --template feature', {
      cwd: testDir,
      encoding: 'utf8',
      env: process.env,
    });
  ...
  test('viewer角色：只读权限验证', async () => {
    ...
    // 先创建PRD供测试
    execSync('prd config set user.role "architect"', { cwd: testDir, env: process.env });
    const createOutput = execSync('prd create --title "观察者测试PRD" --template business', {
      cwd: testDir,
      encoding: 'utf8',
      env: process.env,
    });
  ...
  test('角色切换和权限验证', async () => {
    ...
    // 以architect身份创建PRD
    const createOutput = execSync('prd create --title "权限测试PRD" --template technical', {
      cwd: testDir,
      encoding: 'utf8',
      env: process.env,
    });
    ...
    // 应该能够重新编辑
    const editOutput = execSync(`prd edit ${prdId} --section overview --message "架构师重新编辑"`, {
      cwd: testDir,
      encoding: 'utf8',
      env: process.env,
    });
  ...
  test('敏感操作需要确认', async () => {
    ...
    const createOutput = execSync('prd create --title "删除测试PRD" --template business', {
      cwd: testDir,
      encoding: 'utf8',
      env: process.env,
    });
    ...
    // 版本回滚也需要确认
    execSync('prd config set user.role "architect"', { cwd: testDir, env: process.env });
    const newCreateOutput = execSync('prd create --title "回滚测试PRD" --template technical', {
      cwd: testDir,
      encoding: 'utf8',
      env: process.env,
    });
