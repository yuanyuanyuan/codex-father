...
describe('PRD模板系统集成测试', () => {
  ...
  test('模板字段自定义和验证', async () => {
    ...
    // 使用自定义字段模板创建PRD
    const createApiPrdOutput = execSync(
      'prd create --title "用户管理API" --template api-design',
      { cwd: testDir, encoding: 'utf8', env: process.env }
    )
  ...
  test('模板导入导出功能', async () => {
    ...
    const importOutput = execSync(
      `prd template import --file ${modifiedFile}`,
      { cwd: testDir, encoding: 'utf8', env: process.env }
    )
  ...
  test('模板性能和缓存验证', async () => {
    ...
    execSync(
      'prd create --title "性能测试PRD" --template business',
      { cwd: testDir, encoding: 'utf8', env: process.env }
    )
  ...
  test('模板错误处理和验证', async () => {
    ...
    // 测试创建重复名称的模板
    execSync(
      'prd template create --name "duplicate-test" --title "重复测试"',
      { cwd: testDir, env: process.env }
    )

    expect(() => {
      execSync(
        'prd template create --name "duplicate-test" --title "另一个重复测试"',
        { cwd: testDir, env: process.env }
      )
