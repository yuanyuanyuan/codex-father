...
- 修改的原则：无（新建）
- 新增部分：
  * 五项核心原则：代码质量与可维护性、测试优先开发（TDD）、用户体验一致性、性能与效率要求、安全与可靠性
  * 开发流程与协作规范：代码审查、PR要求、CI、分支策略
  * 质量门禁：性能基准、安全检查、文档完整性、测试覆盖率门禁
  * 治理：宪章权威性、修订程序、合规性审查
- 移除部分：无
- 模板更新状态：
  * ✅ 已更新：.specify/templates/plan-template.md（Constitution Check 部分）
  * ✅ 已检查：.specify/templates/spec-template.md（无需更新）
  * ✅ 已检查：.specify/templates/tasks-template.md（无需更新）
  * ✅ 已检查：AGENTS.md（符合宪章要求）
- 后续 TODO：无
-->
...
- 严格遵循 DRY（Don't Repeat Yourself）原则：识别并消除重复代码，通过抽象和复用提升维护性
- 严格遵循 KISS（Keep It Simple, Stupid）原则：追求简洁设计，拒绝不必要的复杂性
- 严格遵循 YAGNI（You Aren't Gonna Need It）原则：仅实现当前明确需要的功能，避免过度设计
- 统一的代码风格：Shell 脚本遵循 Google Shell Style Guide，TypeScript 遵循项目 ESLint 配置
- 函数和变量命名必须具有描述性：Shell 使用 `snake_case`，TypeScript 使用 `camelCase`
- 关键逻辑必须添加注释说明意图和复杂决策的原因

**理由**：高质量代码是项目长期可持续发展的基石。Codex Father 作为工具集，需要被多人维护和扩展，清晰的代码结构能显著降低协作成本和错误率。
...
**理由**：Codex Father 涉及复杂的指令组合、异步任务管理和 MCP 协议交互，测试是保证系统可靠性的唯一手段。TDD 确保代码从设计之初就是可测试的，避免后补测试的困境。

### 三、用户体验一致性

**必须遵循的规则**：
- CLI 命令必须遵循统一的参数命名和行为模式（如 `--task`、`--dry-run`、`--json` 等）
...
**理由**：安全漏洞和可靠性问题会导致用户数据丢失、隐私泄露或工作流中断。作为执行代码的工具，Codex Father 必须最高优先级保障安全和可靠性。
...
### Pull Request 要求
- PR 标题必须清晰描述变更内容，遵循 Conventional Commits 规范（如 `feat:`, `fix:`, `docs:`, `test:`）
...
### 宪章权威性
本宪章是 Codex Father 项目的最高指导原则，优先级高于其他所有实践、约定和个人偏好。
...
- 代码风格：`.editorconfig`、`eslint.config.js`（TypeScript）、Google Shell Style Guide（Shell）
- 测试指南：`tests/README.md`（如存在）
- 贡献指南：`AGENTS.md`
- 架构文档：`docs/DESIGN-async-mcp.md`

---

**版本**: 1.0.0 | **批准日期**: 2025-09-27 | **最后修订**: 2025-09-27